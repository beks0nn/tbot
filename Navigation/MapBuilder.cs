...
public class MapBuilder
{
    ...
    public void AddMinimap(Mat minimap, int offsetX, int offsetY)
    {
        ...
        // Crop small border
        var innerRect = new Rect(1, 1, miniBgr.Width - 2, miniBgr.Height - 2);
        ...
        var roi = new Rect(_centerX + offsetX, _centerY + offsetY, miniCropped.Width, miniCropped.Height);
    ...
    public Point? FindInitialOffset(Mat currentMinimap, string existingMapPath)
    {
        ...
        int border = 3;
        if (grayMini.Width > border * 2 && grayMini.Height > border * 2)
            grayMini = new Mat(grayMini, new Rect(border, border, grayMini.Width - 2 * border, grayMini.Height - 2 * border));
